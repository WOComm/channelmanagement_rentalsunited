	...
	public static function import_property( $channel , $remote_property_id = 0 , $proxy_id = 0 )
	{
		...
		try {
			...
			if ($remote_property['Property']['IsArchived'] != "true" ) {
				...
				// First we figure out the country based on the Lat/Long
				$siteConfig = jomres_singleton_abstract::getInstance('jomres_config_site_singleton');
				$jrConfig = $siteConfig->get();
				...
				foreach ($property_room_types as $room_type ) {
					...
					try { // It's ok-ish if this fails, the webhook watcher may put the prices right later
						channelmanagement_rentalsunited_import_prices::import_prices( $JRUser->id , $channel , $remote_property_id , $new_property_id , $remote_property['Property']['CanSleepMax'] , $room_type['amenity']->jomres_id );
					}
					catch (Exception $e) {
						logging::log_message("Failed to add property tariffs. Error message : ".$e->getMessage()." -- Remote property id ".$remote_property_id , 'CMF', 'INFO' , '' );
					}
		...
		catch (Exception $e)
		{
			if ($new_property_id > 0 ) {
				...
				logging::log_message("Failed to create new property, so incomplete and removed again. Error message : ".$e->getMessage()." -- Remote property id ".$remote_property_id , 'CMF', 'INFO' , '' );
			}
